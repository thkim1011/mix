...
/**
 * <b>Register class.</b> The register class serves to be an abstraction of the
 * idea of a register.
 * 
 ...
 * purpose are getWord, which converts the register into a Word object using a
		...
		for(i = 5; i >= 1; i--) {
			if(i >= 6 - mySize) {
				...
				if(myBytes[i-1].getValue() != 0) {
					throw new IllegalArgumentException("Inexistent bytes of the register were attempted to be written.");
				}
			}
		}
	}
